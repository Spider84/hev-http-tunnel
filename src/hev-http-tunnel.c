...
static HevTask *task_perfect_server;

static HevHttpSessionBase *session_list;
...
static void perfect_server_task_entry (void *data);
...
int
hev_http_tunnel_init (int tunfd)
{
    ...
    task_perfect_server = hev_task_new (-1);
    if (!task_perfect_server) {
        LOG_E ("Create task perfect server failed!");
        goto exit_free_task_session_manager;
    }

    if (signal (SIGPIPE, SIG_IGN) == SIG_ERR) {
        LOG_E ("Set signal pipe's handler failed!");
        goto exit_free_task_perfect_server;
    }
...
exit_free_task_perfect_server:
    hev_task_unref (task_perfect_server);
...
hev_http_tunnel_run (void)
{
    ...
    task_perfect_server = hev_task_ref (task_perfect_server);
    hev_task_run (task_perfect_server, perfect_server_task_entry, NULL);
...
event_task_entry (void *data)
{
    ...
    hev_task_wakeup (task_perfect_server);
...
lwip_io_task_entry (void *data)
{
    ...
    for (; run;) {
        ...
        if (buf->next) {
            ...
            for (i = 0, p = buf; (i < 256) && p; p = p->next) {
                iov[i].iov_base = p->payload;
                iov[i].iov_len = p->len;
                i++;
            }
...
static void
session_manager_insert_session (HevHttpSession *session)
{
    HevHttpSessionBase *session_base = (HevHttpSessionBase *)session;
...
static void
session_manager_remove_session (HevHttpSession *session)
{
    HevHttpSessionBase *session_base = (HevHttpSessionBase *)session;
...
static void
session_close_handler (HevHttpSession *session)
{
...
{
    HevHttpSession *session;

    if (err != ERR_OK)
        return err;

    session = hev_http_session_new_tcp (pcb, &mutex, session_close_handler);
    if (!session)
        return ERR_MEM;

    session_manager_insert_session (session);
    hev_http_session_run (session);
                  ...
                  const ip_addr_t *addr, u16_t port)
{
    ...
    session = hev_http_session_new_dns (pcb, p, addr, port, &mutex,
...
static void
perfect_server_task_entry (void *data)
{
    const int timeout = 5000;
    const int sc = hev_config_get_server_count ();
    int i, remain, min, pick, delays[sc];
    struct pollfd fds[sc];
...
retry:
    ...
    for (i = 0; i < sc; i++) {
        ...
        delays[i] = 0;
        fds[i].fd = hev_task_io_socket_socket (AF_INET6, SOCK_STREAM, 0);
        if (-1 == fds[i].fd)
            goto exit;

        fds[i].revents = 0;
        fds[i].events = POLLOUT;
        hev_task_add_fd (hev_task_self (), fds[i].fd, POLLOUT);

        srv = hev_config_get_server (i);
        connect (fds[i].fd, (struct sockaddr *)srv->addr, srv->addr_len);
    }

    for (remain = timeout; remain;) {
        remain = hev_task_sleep (remain);
        ...
        for (i = 0; i < sc; i++) {
            ...
            if ((POLLERR | POLLHUP) & revents)
                continue;

            if ((POLLOUT & revents) && !delays[i]) {
                HevConfigServer *srv = hev_config_get_server (i);
                delays[i] = (timeout - remain) * srv->weight / 10;
            }
        }
    }

exit:
    for (i = 0, min = 0, pick = -1; i < sc; i++) {
        close (fds[i].fd);
    ...
    if (pick >= 0) {
        hev_config_set_perfect_server (pick);

        if (LOG_ON_I ()) {
            HevConfigServer *srv = hev_config_get_server (pick);
            LOG_I ("Set perfect server: %s", srv->name);
        }
    }
